// popup.js - Updated Export Button Action
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  openSidebarBtn.addEventListener('click', () => {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => { if (tabs && tabs.length > 0 && tabs[0].id) { chrome.tabs.sendMessage(tabs[0].id, { action: 'openSidebar' }, (response) => { if (chrome.runtime.lastError) { console.warn("Msg Send Error(Open):", chrome.runtime.lastError.message); } else { console.log("Open Resp:", response); window.close(); } }); } else { console.error("No active tab."); } });
  });
  ...
  exportPromptsBtn.addEventListener('click', () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
       if (tabs && tabs.length > 0 && tabs[0].id) {
          ...
          chrome.tabs.sendMessage(tabs[0].id, { action: 'exportSelectedPrompt' }, (response) => {
               if (chrome.runtime.lastError) {
                   console.warn("Msg Send Error(Export Selected):", chrome.runtime.lastError.message);
